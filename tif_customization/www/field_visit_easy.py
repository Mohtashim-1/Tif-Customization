...
def get_context(context):
	...
	if frappe.session.user == "Guest":
		frappe.local.flags.redirect_location = "/login?redirect-to=/field-visit-easy"
		raise frappe.Redirect

	js_url, css_url = _portal_asset_urls()
	context.title = _("Field Visit — Easy Form")
...
def _portal_asset_urls():
	base = "/assets/tif_customization/field_visit_portal"
	app_path = frappe.get_app_path("tif_customization")
	public_root = os.path.join(app_path, "public", "field_visit_portal")
	manifest_path = os.path.join(public_root, ".vite", "manifest.json")

	js_rel = "assets/field_visit_portal.js"
	css_rel = "assets/field_visit_portal.css"
