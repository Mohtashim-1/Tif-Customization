...
def get_context(context):
	...
	context.title = _("The ILM Foundation · Training · Attendance · LMS · Certification")
	...
	if user != "Guest":
		...
		try:
			from tif_customization.tif_customization.api.training_lms import portal_role_for

			role = portal_role_for(user) or "admin"
		except Exception:
			role = "admin"
	context.boot = {
...
def _portal_asset_urls():
	base = "/assets/tif_customization/training_lms"
	app_path = frappe.get_app_path("tif_customization")
	public_root = os.path.join(app_path, "public", "training_lms")
	manifest_path = os.path.join(public_root, ".vite", "manifest.json")

	js_rel = "assets/training_lms.js"
	css_rel = "assets/training_lms.css"
