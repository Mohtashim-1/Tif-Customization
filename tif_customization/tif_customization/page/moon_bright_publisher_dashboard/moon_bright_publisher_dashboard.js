frappe.pages["moon-bright-publisher-dashboard"].on_page_load = function (wrapper) {
	frappe.require("/assets/tif_customization/css/millat_publisher_dashboard.css");

	const SUPPLIER = "Moon Bright Publishers";
	const SUPPLIER_Q = encodeURIComponent(SUPPLIER);
	const MOON_BRIGHT_WAREHOUSE = "Moon Bright Warehouse - TIF";
	const MOON_BRIGHT_WAREHOUSE_Q = encodeURIComponent(MOON_BRIGHT_WAREHOUSE);

	const page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __("Moon Bright Publisher Dashboard"),
		single_column: true,
	});
	...
	if (!window.MoonBrightPublisherDashboard) {
		window.MoonBrightPublisherDashboard = class MoonBrightPublisherDashboard {
			constructor(page) {
				this.page = page;
				this.charts = {};
				this.filters = {
					from_date: "",
					to_date: "",
					warehouse: MOON_BRIGHT_WAREHOUSE,
				};
			}
				...
				this.page.main.html(`
					<div class="millat-dash-root">
						<div class="millat-hero">
							<div>
								<h1 class="millat-hero__title">${__("Moon Bright Publisher Dashboard")}</h1>
								<p class="millat-hero__sub">${__(
									"Complete view of orders placed, material received, deliveries from Moon Bright Warehouse, invoices, and payments for Moon Bright Publishers."
								)}</p>
							</div>
							<div class="millat-hero__links">
								<a href="/app/purchase-order?supplier=${SUPPLIER_Q}">${__("Purchase Orders")}</a>
								<a href="/app/delivery-note?set_warehouse=${MOON_BRIGHT_WAREHOUSE_Q}">${__("Delivery Notes")}</a>
						...
						<div class="millat-kpi-grid" id="moon-kpi-grid"></div>
						...
						<div class="millat-charts-row millat-charts-row--trend">
							<div class="millat-panel">
								...
								<div class="millat-chart-wrap" id="moon-trend-wrap">
									<canvas id="moon-trend-chart"></canvas>
								</div>
							</div>
						</div>
						...
						<div class="millat-charts-row millat-charts-row--split">
							<div class="millat-panel">
								...
								<div class="millat-chart-wrap millat-chart-wrap--sm" id="moon-fulfill-wrap">
									<canvas id="moon-fulfill-chart"></canvas>
								</div>
							</div>
							<div class="millat-panel">
								<h3 class="millat-panel__title">${__("Payment Status (Amount)")}</h3>
								<div class="millat-chart-wrap millat-chart-wrap--sm" id="moon-payment-wrap">
									<canvas id="moon-payment-chart"></canvas>
								</div>
							</div>
						</div>
						...
						<div class="millat-charts-row millat-charts-row--trend">
							<div class="millat-panel">
								...
								<div class="millat-chart-wrap millat-chart-wrap--items" id="moon-items-wrap">
									<canvas id="moon-items-chart"></canvas>
								</div>
							</div>
						</div>
						...
						<div class="millat-tables">
							<div class="millat-panel">
								...
								<div class="millat-table-wrap" id="moon-po-table"></div>
							</div>
							<div class="millat-panel">
								<h3 class="millat-panel__title">${__("Purchase Invoices")}</h3>
								<div class="millat-table-wrap" id="moon-pi-table"></div>
							</div>
							<div class="millat-panel">
								<h3 class="millat-panel__title">${__("Delivery Notes")} — ${MOON_BRIGHT_WAREHOUSE}</h3>
								<div class="millat-table-wrap" id="moon-dn-table"></div>
							</div>
							<div class="millat-panel">
								<h3 class="millat-panel__title">${__("Payments")}</h3>
								<div class="millat-table-wrap" id="moon-pay-table"></div>
							</div>
						</div>
					</div>
				`);
			...
			bind_events() {
				...
				if (!this._resizeBound) {
					...
					$(window).on("resize.moon_bright_dash", () => {
			...
			async refresh() {
				...
				try {
					const r = await frappe.call({
						method:
							"tif_customization.tif_customization.page.moon_bright_publisher_dashboard.moon_bright_publisher_dashboard.get_dashboard_data",
			...
			render_kpis() {
				...
				this.page.main.find("#moon-kpi-grid").html(
					cards
						.map(
							(c) => `
			...
			render_charts() {
				...
				this.upsert_chart("trend", "moon-trend-chart", "moon-trend-wrap", {
					type: "line",
					data: {
						labels: trend.labels || [],
						datasets: [
				...
				this.upsert_chart("items", "moon-items-chart", "moon-items-wrap", {
					type: "bar",
					data: {
	...
	loadCharts()
		.then(() => new window.MoonBrightPublisherDashboard(page).make())
