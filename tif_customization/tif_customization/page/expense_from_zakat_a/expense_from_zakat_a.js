frappe.pages["expense-from-zakat-a"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __("Purchase from Zakat"),
		single_column: true,
	});

	if (!window.PurchaseFromZakatReport) {
		window.PurchaseFromZakatReport = class PurchaseFromZakatReport {
			constructor(page) {
				this.page = page;
				this.filters = {
					from_date: "",
					...
					<div class="zakat-report">
						<div class="zakat-hero">
							<div class="zakat-hero__copy">
								...
								<h2>${__("Purchase from Zakat")}</h2>
								<p>${__(
									"Track Zakat received and how much Zakat-marked money was used for supplier purchase invoices."
								)}</p>
							</div>
							<div class="zakat-hero__side">
								<div class="zakat-actions">
									<a href="/app/account/GENERAL%20-%20ZAKAT%20-%20TIF">${__("Zakat Income Account")}</a>
						...
						<div class="zakat-grid">
							...
							<div class="zakat-panel">
								<div class="zakat-panel__head">
									<div>
										<h3>${__("Supplier-wise Zakat Purchases")}</h3>
										<p>${__("Which suppliers were paid from Zakat-marked entries")}</p>
									</div>
								</div>
						...
						<div class="zakat-panel">
							<div class="zakat-panel__head">
								<div>
									...
									<p>${__(
										"Submitted supplier Payment Entries linked to Purchase Invoices where remarks or paid-from account mention Zakat"
									)}</p>
								</div>
							</div>
						...
						<div class="zakat-panel">
							<div class="zakat-panel__head">
								<div>
									<h3>${__("Zakat Received Details")}</h3>
									<p>${__("Received amount is calculated from credits posted to Zakat income accounts")}</p>
								</div>
							</div>
			...
			async refresh() {
				...
				try {
					const response = await frappe.call({
						method:
							"tif_customization.tif_customization.page.expense_from_zakat_a.expense_from_zakat_a.get_report_data",
						args: { filters: this.filters },
					});
					this.data = response.message || {};
					this.render();
				} catch (error) {
					frappe.msgprint(__("Could not load Purchase from Zakat report"));
				} finally {
			...
			render_kpis() {
				const summary = this.data.summary || {};
				const cards = [
					{ label: __("Zakat Received"), value: this.money(summary.zakat_received), color: "#059669", icon: "↓", hint: __("Posted to Zakat income accounts") },
					...
					{ label: __("Remaining Zakat"), value: this.money(summary.remaining_amount), color: "#2563eb", icon: "=", hint: __("Received minus used for purchases") },
					{ label: __("Suppliers"), value: this.num(summary.supplier_count), color: "#7c3aed", icon: "#", hint: __("Paid from Zakat-marked entries") },
				];
				...
				this.page.main.find("#zakat-usage-card").html(`
					<div class="zakat-usage">
						<div class="zakat-usage__meta">
							<div>
								<div class="zakat-usage__label">${__("Zakat utilization for purchases")}</div>
						...
						<div class="zakat-usage__foot">
							...
							<span>${__("Source")}: ${__("Zakat income credits + Zakat-marked purchase payments")}</span>
						</div>
					</div>
				`);
			}
			...
			render_monthly() {
				this.render_table(
					...
					[
						...
						{ key: "received", label: __("Zakat Received"), fmt: "money", align: "right" },
			...
			render_purchases() {
				this.render_table(
					...
					[
						...
						{ key: "amount", label: __("Zakat Used"), fmt: "money", align: "right" },
			...
			render_received() {
				this.render_table(
					...
					[
						...
						{ key: "account", label: __("Zakat Account"), link: "Account" },
			...
			add_style() {
				if ($("#purchase-from-zakat-style").length) return;
				$("head").append(`
					<style id="purchase-from-zakat-style">
						.zakat-report {
							padding: 4px 0 28px;
	...
	new window.PurchaseFromZakatReport(page).make();
};
