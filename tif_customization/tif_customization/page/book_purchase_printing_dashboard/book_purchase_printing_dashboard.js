frappe.pages["book-purchase-printing-dashboard"].on_page_load = function (wrapper) {
	frappe.require("/assets/tif_customization/css/millat_publisher_dashboard.css");

	const API =
		"tif_customization.tif_customization.page.book_purchase_printing_dashboard.book_purchase_printing_dashboard";
	...
	if (typeof window.BookPurchasePrintingDashboard === "undefined") {
		window.BookPurchasePrintingDashboard = class BookPurchasePrintingDashboard {
			constructor(page) {
				this.page = page;
				this.charts = {};
			...
			async load_suppliers() {
				const r = await frappe.call({ method: `${API}.get_supplier_options` });
				this.supplier_options = r?.message || [];
				if (!this.filters.supplier && this.supplier_options.length) {
					this.filters.supplier = this.supplier_options[0].supplier;
					this.filters.warehouse = this.supplier_options[0].warehouse || "";
				}
			}

			get_supplier_config() {
				return (
					this.supplier_options.find((o) => o.supplier === this.filters.supplier) ||
					this.supplier_options[0] ||
					{}
				);
			}

			render_layout() {
				const supplier_opts = this.supplier_options
					.map(
						(o) =>
							`<option value="${frappe.utils.escape_html(o.supplier)}">${frappe.utils.escape_html(
								o.label || o.supplier
							)}</option>`
					)
					.join("");
				...
				this.page.main.html(`
					<div class="millat-dash-root">
						<div class="millat-hero">
							<div>
								<h1 class="millat-hero__title" id="bpp-hero-title">${__("Book Purchase & Printing Dashboard")}</h1>
								<p class="millat-hero__sub" id="bpp-hero-sub"></p>
							</div>
							<div class="millat-hero__links" id="bpp-hero-links"></div>
						</div>

						<div class="millat-toolbar">
							<div class="filter-field">
								<label>${__("Supplier")}</label>
								<select class="form-control input-supplier">${supplier_opts}</select>
							</div>
							<div class="filter-field">
						...
						<div class="millat-tables">
							...
							<div class="millat-panel millat-panel--dn" id="bpp-dn-panel">
								<h3 class="millat-panel__title" id="bpp-dn-title">${__("Delivery Notes")}</h3>
				...
				if (this.filters.supplier) {
					this.page.main.find(".input-supplier").val(this.filters.supplier);
				}
			}

			update_header() {
				const cfg = this.get_supplier_config();
				const supplier = cfg.supplier || this.filters.supplier || "";
				const warehouse = cfg.warehouse || "";
				const supplier_q = encodeURIComponent(supplier);

				this.page.main.find("#bpp-hero-sub").text(
					__(
						"Orders placed, material received, invoices, payments{0} for {1}.",
						[
							warehouse ? `, and deliveries from ${warehouse}` : "",
							supplier,
						]
					)
				);

				let links = `<a href="/app/purchase-order?supplier=${supplier_q}">${__("Purchase Orders")}</a>
					<a href="/app/purchase-invoice?supplier=${supplier_q}">${__("Purchase Invoices")}</a>`;
				if (warehouse) {
					links += `<a href="/app/delivery-note?set_warehouse=${encodeURIComponent(warehouse)}">${__(
						"Delivery Notes"
					)}</a>`;
				}
				this.page.main.find("#bpp-hero-links").html(links);

				const $dnPanel = this.page.main.find("#bpp-dn-panel");
				if (warehouse) {
					$dnPanel.show();
					this.page.main.find("#bpp-dn-title").text(`${__("Delivery Notes")} — ${warehouse}`);
				} else {
					$dnPanel.hide();
				}
			}
			...
			bind_events() {
				...
				this.page.main.find(".input-supplier").on("change", function () {
					const cfg = me.supplier_options.find((o) => o.supplier === this.value);
					me.filters.supplier = this.value;
					me.filters.warehouse = cfg?.warehouse || "";
					me.update_header();
					me.refresh();
				});
				if (!this._resizeBound) {
					this._resizeBound = true;
					$(window).on("resize.bpp_dash", () => {
	...
	const page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __("Book Purchase & Printing Dashboard"),
		single_column: true,
	});

	loadCharts()
		.then(() => new window.BookPurchasePrintingDashboard(page).make())
