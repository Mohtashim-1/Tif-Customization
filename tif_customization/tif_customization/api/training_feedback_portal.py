...
def _get_attendee_by_token(token):
	...
	row = frappe.db.get_value(
		...
		["name", "parent", "attendee_name", "email", "school_organization", "designation", "feedback_submitted"],
		as_dict=True,
	)
	if not row:
...
def get_training_feedback_context(token):
	...
	if attendee.feedback_submitted or frappe.db.exists(
		...
		return {
			...
			"training_date": field_visit.training_date,
			"trainer_name": field_visit.training_trainer_name,
			"venue_name": field_visit.training_venue_name,
